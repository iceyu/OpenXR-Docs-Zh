...
一般XR 应用程序会希望向用户呈现渲染图像。
为此，运行时提供了 以将图像组织为 swapchains 的 方式 用于要渲染的应用程序。
运行时 must: 允许应用程序创建多个 swapchains。

运行时支持的 swapchains 图像格式由 flink:xrEnumerateSwapchainFormats 函数指定.
如果可能，运行时 should: 支持 code:R8G8B8A8 and code:R8G8B8A8 code:sRGB formats

Swapchain images can: be 2D or 2D Array.


涉及 提交层组合 的 渲染操作 应该 是由假定 的运行时中内部执行，此运行时在线性颜色空间。


在 sRGB 颜色空间中提交的图像必须使用特定于 API 的创建 sRGB 格式(e.g. ename:DXGI_FORMAT_R8G8B8A8_UNORM_SRGB,
ename:GL_SRGB8_ALPHA8, ename:VK_FORMAT_R8G8B8A8_SRGB) 以在被运行时读取时应用自动的
sRGB-to-linear 的转换。
所有其他格式将被视为线性值。
...
[open,refpage='XrSwapchainUsageFlags',desc='Swapchain usage flags',type='flags',xrefs='xrCreateSwapchain xrEnumerateSwapchainFormats']
--
...
The slink:XrSwapchainCreateInfo passed to flink:xrCreateSwapchain should
match the intended usage or else undefined behavior may result when the
application works with the images.
...
[open,refpage='XrSwapchainCreateFlags',desc='Swapchain creation flags',type='flags',xrefs='xrCreateSwapchain XrSwapchainCreateInfo']
--
The pname:createFlags are a combination of the following:
...
应用程序 使用 flink:xrLocateViews 来获取 在一个合成投影层中用于渲染每个视图所需的 观察者姿态和投影参数。
...
[open,refpage='XrViewStateFlags',desc='View state flags',type='flags',xrefs='xrLocateViews XrViewState XrView']
--
...
应用程序通过调用 flink:xrWaitFrame 将它的渲染循环与运行时进行同步。
...
[open,refpage='XrCompositionLayerFlags',desc='Composition layer flags',type='flags',xrefs='XrCompositionLayerProjection XrCompositionLayerQuad']
--

The elink:XrCompositionLayerFlagBits bitfield is specified as:
include::../../generated/api/enums/XrCompositionLayerFlagBits.txt[]

elink:XrCompositionLayerFlags specify options for individual composition
layers.
