...
OpenXR的应用程序使用XrActions与输入设备进行通信。
Actions在初始化时被创建。在之后用于请求输入设备的状态，创建 action spaces，或控制触觉反馈事件。
输入action处理代表应用程序感兴趣的需要获取状态的 'actions'，而不是与硬件直接关联。

例如，当与一个菜单交互时，OpenXR的应用程序在启动时创建一个 actionname:menu_select 的action 然后向OpenXR询问action的状态，而不是应用程序直接查询A键的状态。

应用程序建议 action 通过一个已知的 <<semantic-path-interaction-profiles,
interaction profile>> 分配给输入设备上一个特定的输入源。
但是在运行时中，要有能力根据用户的喜好，输入设备的可用性或其他理由选择不同的控制方式。

这一抽象保证应用程序可以在多种多样的输入设备上运行最大化用户的可接入性。
...
Action sets 是应用程序定义的 actions 的集合。
他们通过调用 flink:xrAttachSessionActionSets 被附加一个给定 slink:XrSession 上。
根据当前应用程序的上下文，应用程序通过 flink:xrSyncActions 将他们使能或禁用。

例如，游戏可能有 一组 actions  用于控制人物，另一组 actions 用于引导菜单系统。当这些 actions 被分组为两个
 slink:XrActionSet 句柄后，他们就可以通过一个函数调用被选择性的使能或禁用。

当 actions 创建的时候 需要传递一个 关于他们的 slink:XrActionSet 句柄

Action sets 的创建 通过调用 flink:xrCreateActionSet:
--
...
当多个 actions 被绑定到同样的输入源上，每一个 action set 中的 pname:priority 来确定哪一个绑定被被抑制。
...
两个actions即使有不同的组件分割，只要他们使用相同 <<semantic-path-input, identifier and optional location>> 路径分割
就被认为是绑定了相同的输入源。
