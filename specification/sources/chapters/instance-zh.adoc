...
OpenXR 实例 是一个允许 OpenXR 应用程序与 OpenXR运行时进行通信的对象。
应用程序通过调用 flink:xrCreateInstance 并接收作为结果的 slink:XrInstance 对象句柄 来完成这种通信。


slink:XrInstance 对象存储和跟踪 OpenXR 相关的应用程序的状态，而不是在应用程序的全局地址中存储任何此类状态。
...
If pname:properties is code:NULL, then the number of extensions properties
available is returned in pname:propertyCountOutput.
Otherwise, pname:propertyCountInput must point to a variable set by the user
to the number of elements in the pname:properties array.
...
[open,refpage='XrInstanceCreateFlags',desc='Instance Creation Flags',type='flags',xrefs='xrCreateInstance XrInstanceCreateInfo']
--
The elink:XrInstanceCreateFlags include:

include::../../generated/api/enums/XrInstanceCreateFlagBits.txt[]

There are currently no instance creation flags.
