...
--
在OpenXR API的多个地方使用多部分版本编号

include::../../generated/api/basetypes/XrVersion.txt[]

在每个此用例中，API的major版本编号，minor版本编号以及patch版本编号被封装为一个64-bit整数(basetype:XrVersion)
...
OpenXR API 意图在使用多个主机线程时提供可扩展的性能。

所有的函数 must: 支持从多线程并发调用。但是 某些参数或参数的组成部分被定义为需外部同步。
这意味着调用者 must: 保证在给定的时间内，只有一个线程在使用这样的参数。
更准确的说，函数使用简单的存储来更新软件结构表示的对象。
被声明为外部同步的参数 在 host 执行函数的任何时候 may: 拥有自己的软件结构被更新。

如果两个函数对相同的对象进行操作，并且至少其中一个函数声明这个对象为外部同步的，则调用者 must: 
保证不仅这些函数不是同时执行的，还要在必要时使用一个适当的内存屏障将两个函数进行隔离。

对于所有的销毁对象句柄的函数，应用程序 must: 在外部同步 对象句柄参数以及任何子句柄。
...
OpenXR被设计为一个分成的API，这意味这用户或应用程序 may: 在应用程序和运行时实现之间插入 API layers。
...
=== 句柄
句柄表示在应用程序中由运行时分配的对象。
句柄是对象不透明的标识符，其生命周期由应用程序通过创建函数和销毁函数控制。

Example handle types include slink:XrInstance, slink:XrSession, and
slink:XrSwapchain.

对于给定的应用程序，未销毁的句柄是唯一，但 may: 被销毁后被重用。
除非另有说明，一个成功的句柄创建函数调用返回一个新的唯一句柄
除非另有说明，句柄会在其父句柄被销毁时隐式的销毁。

应用程序  may: 在父句柄销毁前显式的销毁句柄 ，并且为了节省资源，当不再需要时 should: 这么做。

当有效句柄被需要的时候， 运行时 may: 检测  dlink:XR_NULL_HANDLE 和其他无效句柄的传递，并且返回  ename:XR_ERROR_HANDLE_INVALID。

然而，除非另有说明，运行时不需要这样做。因此，任何无效句柄的如此使用 may: 导致未定义的行为。

当一个函数有一个可选的句柄参数时， dlink:XR_NULL_HANDLE  must: 被使用，除非传递的是一个有效句柄。

所有带有句柄参数的函数函数 may: 返回 ename:XR_ERROR_HANDLE_INVALID.

句柄形成一个层次结构，子句柄受父句柄的有效性和的生命周期的影响。

例如，创建一个 slink:XrSwapchain 句柄，应用程序必须调用 flink:xrCreateSwapchain 并传递 一个
slink:XrSession 句柄。因此 slink:XrSwapchain 是 slink:XrSession 的一个子句柄。
...
A two-call idiom may: be employed。

First calling fname:xrFunction (with a
...
时间由表示为纳秒的64-bit有符号整数表示(basetype:XrTime).
时间的流逝 must: 是单调的，而不是实时的 (i.e. wall clock
time)。
因此，时间总是以恒定的速率增加，并且不受时钟更改、时区、夏令时等。
